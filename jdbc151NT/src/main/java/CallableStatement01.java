...
public class CallableStatement01 {
    public static void main(String[] args) throws SQLException {
        ...
        CallableStatement callableStatement = connection.prepareCall("{? = call selamlama(?)}");
                          //ilk soru işareti olan yer String sql de return ile end arasındaki yerin hepsini kapsar
                          //2. soru işareti selamlama metodunun içerisine girilen veriyi simgeler

        //4. Adım: Return için registerOutParameter() methodunu, parametreler için ise setString, setInt gibi methodları kullanınız

        callableStatement.registerOutParameter(1, Types.VARCHAR); //yukarıda sql:"{?= call selamlama(?)}") ilk soru işareti olan yeri söyler
        callableStatement.setString(2, "Ayşe"); // Yukarıda selamlamanın içerisindeki soru işaretine Ayşeyi koyar
        ...
        System.out.println(callableStatement.getObject(1));
                 // Merhaba Ayşe, nasılsın? yazdırır!!!!
